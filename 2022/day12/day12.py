class Node:
    def __init__(self, position: tuple[int, int], real_cost: int = 0):
        self.postion = position
        self.real_cost = real_cost
        self.heuristic_cost = real_cost
        self.parent = None
...
def calculate_position(area: list[list[str]], character: str) -> list[int]:
...
'''
def calculate_path(area: list[list[str]]) -> int:
    start = calculate_position(area, 'S')
    end = calculate_position(area, 'E')
...
def main() -> None:
    ...
    path = calculate_path(area)
    
    print(f'The fewest step required are {len(path)-1}')
